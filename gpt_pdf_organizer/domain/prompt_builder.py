...
def build_query_from_content(content: str) -> str:
    return (f"Based on the following text extract of the first pages of a PDF file, extract: "
            ...
            f"in json dictionary format like this: {{ \"{OrganizingFields.CONTENT_TYPE.name.lower()}\":" 
            f"\"{OrganizingFields.CONTENT_TYPE.name}\", \"{OrganizingFields.AUTHOR.name.lower()}\": "
            f"\"{OrganizingFields.AUTHOR.name}\", \"{OrganizingFields.YEAR.name.lower()}\": \"{OrganizingFields.YEAR.name}\"," 
            f"\"{OrganizingFields.TITLE.name.lower()}\": \"{OrganizingFields.TITLE.name}\"}}. "
